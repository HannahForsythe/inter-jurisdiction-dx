...
# as of Jan 1., on the MenACWY antigen of the 132321 series. 
...
#-----------------------------------------------------------------------------#
MCV_state <- num_denom_132321 %>% 
  group_by(in_state, year) %>%
  summarise(
    pop = n_distinct(person_id),
    utd_diff_n = sum(UTD_diff_MCV, na.rm = TRUE)
  ) %>% 
  mutate(
...
write.csv(MCV_state, 
          file = paste0("output/3_coverage/MCV_state-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
write.csv(MCV_cnty, 
          file = paste0("output/3_coverage/MCV_cnty-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
write.csv(MCV_sex, 
          file = paste0("output/3_coverage/MCV_sex-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
write.csv(MCV_sex_bc, 
          file = paste0("output/3_coverage/MCV_sex_bc-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
write.csv(MCV_race1, 
          file = paste0("output/3_coverage/MCV_race1-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
write.csv(MCV_race1_bc, 
          file = paste0("output/3_coverage/MCV_race1_bc-", home_st, "-", format(Sys.Date(), "%Y%m%d"),".csv"),
          row.names = FALSE)
